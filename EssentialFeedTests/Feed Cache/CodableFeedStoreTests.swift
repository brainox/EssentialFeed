...
class CodableFeedStoreTests: XCTestCase {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    switch (firstResult, secondResult) {
                    case let (
                        .found(firstFoundItem, firstFoundTimestamp),
                        .found(secondFoundItem, secondFoundTimestamp)
                    ):
                        XCTAssertEqual(firstFoundItem, feed)
                        XCTAssertEqual(firstFoundTimestamp, timestamp)
                        
                        XCTAssertEqual(secondFoundItem, feed)
    ...
    private func testSpecificStoreURL() -> URL {
        FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!.appending(path: "\(type(of: self)).store", directoryHint: .isDirectory)
    }
